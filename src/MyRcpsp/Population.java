...
public class Population {
	private Individual population[];     //数组每个位置上存的个体  合起来称总群
	...
	public Population(int populationSize,List<Task> TaskSet,int totalTime,int []resourcesLimit){	//构造数量为populationSize的种群
		this.population=new Individual[populationSize];			// 种群内每个个体为满足紧前紧后约束的随机序列
		for(int individualCount=0;individualCount<populationSize;individualCount++){
			Individual in=new Individual(TaskSet);       //生成序列后还为每个个体设置总时间    还有每个资源限量
			in.setTotalTime(totalTime);
			in.setResourcesLimit(resourcesLimit);
			in.setId(individualCount);
			population[individualCount]=in;					
		}
	}
